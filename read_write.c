...
ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	ssize_t key_size = vfs_getxattr(file->f_path.mnt->mnt_idmap, file->f_path.dentry, 
                                "user.cw3_encrypt", key_buf, sizeof(key_buf));
	if (key_size > 0) {
		key_buf[key_size] = '\0';
		int xor_key = simple_strtol(key_buf, NULL, 10); 
